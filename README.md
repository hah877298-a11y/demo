...
## 10. 硬件 SPI 读写 W25Q64 + OLED 显示

**项目目录**：[10_SPI_W25Q64_HW](./10_SPI_W25Q64_HW)

**实验简介**：使用 STM32 硬件 SPI1 驱动 W25Q64（8MB SPI Flash），通过 JEDEC ID 指令读取芯片厂商和容量信息，并在 SSD1306 OLED 上显示。实验同时挂载 SPI 和 I2C 两条总线，互不干扰。

**硬件接线**：

| Blue Pill | W25Q64 | OLED (SSD1306) |
|:---------:|:------:|:--------------:|
| 3.3V | VCC | VCC |
| GND | GND | GND |
| PA4 | CS | — |
...
> SPI 总线（W25Q64）和 I2C 总线（OLED）各占独立引脚，同时运行互不干扰。

**软件架构**：

```text
┌─────────────────────────────────────────┐
│ main.c        ← 初始化 → 读 ID → OLED 显示 │
├─────────────────────────────────────────┤
│ w25q64.c      ← JEDEC ID / 读写 / 擦除     │
│ OLED.c        ← SSD1306 字符串/数字显示     │
├─────────────────────────────────────────┤
│ spi.c         ← 硬件 SPI1 字节收发          │
│ i2c.c         ← 软件 I2C 模拟时序           │
├─────────────────────────────────────────┤
│ PA4~PA7 (SPI) │ PB10~PB11 (I2C)            │
└─────────────────────────────────────────┘
```
...
| 阶段 | 文件 | 核心内容 |
|:----:|------|---------|
| 1 | `spi.c` / `spi.h` | 硬件 SPI1 初始化（Mode 3, 256 分频）+ `MySPI_SwapByte` |
| 2 | `w25q64.c` / `w25q64.h` / `w25q64_Ins.h` | JEDEC ID 读取（0x9F）、页编程、扇区擦除、读数据 |
| 3 | `i2c.c` / `i2c.h` | 软件模拟 I2C（PB10/PB11 开漏输出，~100kHz） |
| 4 | `OLED.c` / `OLED.h` | SSD1306 页寻址模式、字符串/数字显示 |
| 5 | `main.c` | 综合调度：I2C→OLED→SPI→W25Q64 初始化 → 显示芯片 ID |

**核心知识点**：

- **SPI 四线制**：CS（片选）、SCK（时钟）、MOSI（主出从入）、MISO（主入从出），全双工通信
- **硬件 SPI vs 软件 SPI**：硬件 SPI 由外设自动处理时序，CPU 只读写数据寄存器
- **JEDEC ID 指令**：0x9F 为标准 JEDEC 厂商识别指令，返回厂商 ID + 容量 ID，无需地址字节
- **片选信号**：CS 拉低选中芯片，通信完成后拉高释放，多个 SPI 设备可共享 SCK/MOSI/MISO
- **双总线共存**：SPI 和 I2C 使用不同 GPIO 引脚，可同时运行
...
> 注：不同批次的兼容芯片 JEDEC ID 可能不同（如 MF=0x28, DEV=0x23），读写擦除功能不受影响。

**踩坑记录**：

- OLED 不亮 → 面包板电源轨中间有断口，OLED 的 VCC/GND 改直飞 STM32 解决
- W25Q64 ID 读不准 → 原代码 ReadID 用 0x90 指令少发一个地址字节，换 JEDEC 0x9F 指令（无需地址）后稳定
- I2C 初始化缺失 → 原 `main.c` 只调用了 `OLED_Init()` 但未先调 `SoftI2C_Init()`，GPIO 没有初始化导致 I2C 总线无信号

**开发环境**：

- IDE: Keil MDK (uVision5)，ARMCC V5
- 硬件: STM32F103C8T6 (Blue Pill)
- 存储: W25Q64 (8MB SPI Flash，兼容品)
- 显示: SSD1306 0.96 寸 OLED（128×64, I2C 地址 0x3C）
- 库: STM32 标准外设库 V3.5.0
